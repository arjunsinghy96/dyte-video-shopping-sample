...
const SupportLiveMeeting: React.FC = () => {
  ...
  const setupDyteMeeting = async () => {
    const resp = await axios.post(`http://localhost:8000/live-shopping/live-requests/${meeting_id}/start/`)
  ...
  return (
    <div>
      ...
      {meeting && state.show_meeting && <DyteMeeting meeting={meeting} showSetupScreen={false} />}
    </div>
  )
}

export default SupportLiveMeeting
